				...
				<div class="col-md-10">
					<div class="card">
						...
						<div class="card-body">
							...
							<!-- 공지사항 등록 폼 -->
							<form action="/egg/registerBoard" method="post" enctype="multipart/form-data" id="boardForm">
								<c:if test="${status eq 'u' }">
							...
							<div class="float-end mb-3 me-3">
								...
								<c:choose>
									<c:when test="${status eq 'u'  }">
										<a href="/egg/boardDetail?bbsTyCd=${bbsTyCd }&bbsNo=${boardVO.bbsNo}" class="btn btn-secondary">취소</a>
									</c:when>
									<c:otherwise>
										<a href="/egg/board?bbsTyCd=${bbsTyCd}" class="btn btn-secondary">취소</a>
									</c:otherwise>
								</c:choose>
							</div>
...
<script type="text/javascript">
	$(function() {
		...
		$("#boardRegisterBtn").on("click", function(e) {
			...
			if ($(this).text() == "수정") {
				$("#boardForm").attr("action", "/egg/updateBoard");
			}
