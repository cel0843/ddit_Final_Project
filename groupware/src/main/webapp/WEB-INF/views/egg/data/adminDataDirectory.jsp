...
<body>
    <div class="row">
        <div class="d-flex">
           <a href="/admin/adminDataMain"><h3 class="ri-drive-line">공용 자료실</h3></a>
        </div>
    ...
    <div class="row">
        ...
        <div class="modal fade" id="bs-example-modal-sm" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-sm">
                <form action="/admin/createCommonFolder" method="post" id="folderForm">
        ...
        <div class="modal fade" id="bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="/admin/adminFileupload" method="post" id="drop" enctype="multipart/form-data">
                            <div class="hidden">
                                <input name="fldTyCd" type="hidden" value="P">
                            </div>

                            	업로드할 폴더
                            <select class="form-select mb-3" name="fldNo">
                        	<c:forEach items="${dataList }" var="data">
                                    <option value="${data.fldNo }">
                                        ${data.fldNm }
                                    </option>
                           </c:forEach>
                            </select>
			...
			<c:choose>
				...
				<c:otherwise>
				    <div class="email-menu-list ">
		                <c:forEach items="${dataList }" var="data">
		                   <form action="/admin/delFolder" method="post" class="delFForm">
		                    <div class="d-inline-block">
		                        <a class=" ri-folder-5-fill" onclick="" href="/admin/adminFileList?fileGroupNo=${data.fileGroupNo}&fldNo=${data.fldNo}">
		                            ${data.fldNm }</a> 
		                    </div>
		                    <div class="d-inline-block">
...
<!--             </div> -->
		...
		<div class="row col-md-10 text-center">
                <div class="row col-md-12  text-center" id="fileLocation">
                    <c:choose>
                        <c:when test="${empty allFileList }">
                            <div class="col-sm-12" align="left">
                               <p>조회할 파일이 존재하지 않습니다.</p>
                            </div>
                        </c:when>
                        <c:otherwise>
                            <c:forEach items="${allFileList }" var="dataVO">
                                    ...
                                    <div class="col-lg-2 col-md-4 col-sm-6">
                                        <div class="card hovertext bg-light" data-hover="${file.fileOrgnlNm}[${file.fileFancysize}]">
                                            ...
                                            <c:forTokens var="token" items="${fileNm }" delims="." varStatus="status">
                                                ...
                                                <div class="m-1">
                                                    <a class="d-inline-block" href="${pageContext.request.contextPath}/admin/download?fileGroupNo=${file.fileGroupNo}&fileNo=${file.fileNo}" target="_blank" class="">
                            ...
                            <div class="col-lg-2 col-md-4 col-sm-6 last-item" data-file-id="${lastFileId}"> </div>
                        </c:otherwise>
                    </c:choose>
					<form action="/admin/delete" method="post" class="delForm" id="delFrm">
...
//             } // if end
                ...
                data.forEach(function(dataVO) {
                    if (dataVO.attachFileList && dataVO.attachFileList.length > 0) {
                        dataVO.attachFileList.forEach(function(file, index) {
	                        ...
	                        <div class="col-lg-2 col-md-4 col-sm-6 fileList">
	                            <div class="card  hovertext bg-light"  data-hover="\${file.fileOrgnlNm}[\${file.fileFancysize}]">
	                                <div class="display-1 text-center">
	                                    \${fileIcon}
	                                </div>
	                                ...
	                                <div class="card-footer text-center">
		                                <a class="d-inline-block" href="${pageContext.request.contextPath}/admin/download?fileGroupNo=\${file.fileGroupNo}&fileNo=\${file.fileNo}" target="_blank">
	                                    <h2><i class="ri-download-2-line"></i></h2></a>
	                                    <sec:authorize access="hasAnyRole('ROLE_ADMIN')">
 ...
 $("#fileInput").on("change", function(e) {
		...
		// 무한 스크롤 트리거 요소
		const targetElement = document.querySelector('.last-item'); // 마지막 아이템을 가리키는 요소 선택
		
		// 관찰 시작
		observer.observe(targetElement);
		
		function loadMoreFiles() {
		    // 마지막 파일 ID를 구하는 로직 (예시)
		    lastFileId = document.querySelector('.last-item').dataset.fileId;
		
		    // 로딩 인디케이터 표시
		    loadingIndicator.style.display = 'block';
		
		    // AJAX 요청
		    fetch('/api/loadMoreFiles?lastFileId=' + lastFileId)
		        .then(response => response.json())
		        .then(data => {
		            // 서버에서 받은 데이터를 이용하여 HTML 생성
		            const newFilesHtml = data.map(file => {
		                // 파일 HTML 생성 (기존 코드 참고)
		                return /* 파일 HTML */;
		            }).join('');
		            ...
		            fileLocation.insertAdjacentHTML('beforeend', newFilesHtml);
		
		            // 마지막 아이템 업데이트
		            targetElement = fileLocation.querySelector('.last-item');
		            observer.observe(targetElement); // 새로운 마지막 아이템 관찰
